...
class KoreanTokenizer(object):
    ...
    def add(self, trie_dict, fromPosData, wordPos, endPos, wordID, type_, dict=None):
        ...
        for idx in range(0, fromPosData.count):
            ...
            if self.verbose:
                print("      fromIDX=" + str(idx) + ": cost=" + str(cost) + " (prevCost=" + str(fromPosData.costs[idx]) + " wordCost=" + str(wordCost) + " bgCost=" + str(
                    self.conn_costs.get(fromPosData.lastRightID[idx], leftID)) +
        ...
        if self.verbose:
            print("      + cost=" + str(leastCost) + " wordID=" + str(wordID) + " leftID=" + str(leftID) + " leastIDX=" + str(leastIDX) + " toPos=" + str(endPos) + " toPos.idx=" + str(
                self.positions.get(endPos).count))
    ...
    def backtrace(self, endPosData, fromIDX):
        ...
        if self.verbose:
            print("\n  backtrace: endPos=" + str(endPos) + " pos=" + str(self.pos) + "; " + str(self.pos - self.last_backtrace_pos) + " characters; last=" + str(
                self.last_backtrace_pos) + " cost=" + str(endPosData.costs[fromIDX]))
